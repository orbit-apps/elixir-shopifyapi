defmodule ShopifyAPI.REST.TagTest do
  ...
  describe "Tags.encode and decode" do
    property "single tag test" do
      check all tag_name <- string(:alphanumeric),
                tag_name != "",
    ...
    property "mixed tags collection" do
      check all complex_tags <- list_of({string(:alphanumeric), string(:alphanumeric)}),
                complex_tags != [],
                simple_tags <- list_of(string(:alphanumeric)) do
